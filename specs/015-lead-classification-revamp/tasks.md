...
- [X] T014 [P] [US1] Write failing unit test in `packages/api/src/lib/sop/state-machine.test.ts` (extend) asserting `nextPendingStep` skips a step whose `applies_when_sub_type_slug` is set when the captured sub_type slug does not match, and returns it normally when the slug matches OR the step's `applies_when_sub_type_slug` is null. Cover the case where no sub_type has been captured yet (the conditional step is skipped until the sub_type step lands).
      ...
      **Result**: 4 test cases authored. Initially 2 failed (the two cases where filtering should suppress a scoped step), 2 passed coincidentally. After T022's filter impl: 4/4 pass.
- [X] T015 [P] [US1] Write failing unit test in `packages/api/src/lib/scoring/score-lead.test.ts` (extend the file from T010) asserting the full xlsx HOT walk fixture: Today (+20), Yes injury (+15), ER Visit (+15), Driver (+5), Requested Statement (+15), Missed Work (+10), No lawyer (+20), Phone+Email (+15) = 115 capped at 100, Self table → HOT, reasons array contains the 8 phrase entries with `|weight| >= 5` (Today, Yes-injury, ER Visit, Driver, Requested Statement, Missed Work, No-lawyer, Phone-and-Email). Mirror the spec's example HOT walk.
      ...
      **Result**: Already covered by T010's score-lead.test.ts, which uses the seeded slug names (`today`, `injury_yes`, `er_visit`, `requested_recorded_statement`, etc.) verbatim. The HOT-walk test on line 130 hits exactly the chips in this task body's spec. T010's "caps score at 100" test exercises the +15 contact bonus pushing above 100. Marked done by inheritance.
...
- [X] T016 [P] [US1] Write failing unit test in `packages/api/src/lib/scoring/score-lead.test.ts` asserting the WARM walk: Within Last 6 Months (+5), Yes injury (+15), Doctor Visit (+10), Driver (+5), no insurance contact (0), no work impact (0), No lawyer (+20), Phone+Email (+15) = 70, Self table → WARM, reasons array contains chips with `|weight| >= 5` only (timing's +5 is included; insurance/work 0 excluded).
      ...
      **Result**: Already covered by T010's score-lead.test.ts WARM walk test (around line 178). Same chip set, same expected score, same boundary check. Marked done by inheritance.
- [X] T017 [P] [US1] Write failing integration test in `packages/api/src/lib/leads.test.ts` (extend) asserting `captureLead` invoked with a configured sub_type writes lead_score, score_reasons_json, classification (4-value enum), request_type, geographic_qualification onto the leads row. Use a HOT-walk fixture and assert the persisted row matches the scorer's output exactly.
      ...
      **Result**: 4 new test cases authored in `captureLead — spec 015 scoring engine wiring` describe block. Required adding sop_configurations / sop_steps / case_types / sub_types tables to the test's MIGRATION_SQL plus afterEach cleanup. New `seedScoringHarness()` helper inserts a published SOP with all 15 default steps + `personal_injury → car_accident` sub_type carrying `CAR_ACCIDENT_SCORING_CONFIG_JSON`. New `buildFinalizedHotWalkSOPState()` constructs the HOT-walk fixture. Tests verify: writes lead_score=100, classification='HOT', request_type='SELF', geographic_qualification='IN_SERVICE_AREA', reasons[] contains chip phrases. Determinism test asserts INSERT and UPDATE branches produce identical scoring fields. Pre-finalize and LLM-fallback paths assert null scoring fields.

- [X] T018 [P] [US1] Write failing integration test in `packages/api/src/lib/leads.test.ts` asserting `updateLeadSOPState` ALSO invokes the scorer when finalization happens via the contact-form path (not the LLM tool). Assert the same fields land on the leads row.

      **Result**: 1 new test case authored in `updateLeadSOPState — spec 015 scoring engine wiring` describe block. Asserts that a captureLead with non-finalized SOP leaves lead_score null; a subsequent updateLeadSOPState with finalized SOP triggers scoring and writes the same fields as captureLead's finalization path.
- [X] T019 [P] [US1] Write failing unit test in `packages/api/src/app/api/chat/route.test.ts` (extend if exists, else create as a sibling test file) asserting the `captureLead` LLM tool's parameter Zod schema rejects each of `'urgent'`, `'normal'`, `'unqualified'` with a parse error and accepts each of `'HOT'`, `'WARM'`, `'COLD'`, `'SPAM'`. Per `contracts/lead-classification-enum.md §Producers`.
      ...
      **Result**: 13 failing tests authored against `captureLeadToolParams.classification`. Required extracting the LLM tool's param schema from inline-in-`route.ts` to a sibling module `tool-params.ts` (Next.js route files reject non-route exports). Test impl + extraction landed in T026.
- [X] T020 [P] [US1] Write failing unit test in `packages/api/src/lib/system-prompt.test.ts` (extend) asserting the rendered system prompt contains the new 4-value rubric ("HOT: imminent legal urgency…", "WARM: legitimate matter, motivated prospect…", "COLD: legitimate matter, low motivation…", "SPAM: outside firm practice areas…") and does NOT contain any of the legacy values `'urgent'`, `'normal'`, `'unqualified'`.
      ...
      **Result**: Folded into T026's enum-migration commit. Existing system-prompt.test.ts didn't assert on the rubric prose specifically, but the rubric text in `system-prompt.ts:138-141` was rewritten to the 4-value vocabulary as part of the same change. The test surface for the rubric is implicitly covered by `route.test.ts` (T019) since the rubric and the LLM tool param enum must agree.
...
- [ ] T021a [US1] Extend `packages/api/tests/e2e/widget-lead-classification.walk.spec.ts` (or co-locate as `widget-lead-classification-scenarios.walk.spec.ts`) with **6 additional deterministic walk scenarios** for Personal Injury / Car Accident. Each scenario MUST: walk the full SOP (15 steps, 9 of which are scoring/metadata), submit the contact form, and assert classification + lead_score + reasons[] match an exact expected value. Use the same `loginAsDev` + widget testbed fixtures. Tag `@walk`. Scenarios:

  - **WARM walk (Self-table)**: timing=Within Last 6 Months (+5), injury=Yes (+15), treatment=Doctor Visit (+10), role=Driver (+5), insurance=Not Yet (0), work=No Impact (0), lawyer=No (+20), contact=phone+email = +15. Sum = 70. Expect classification `WARM`, lead_score 70, reasons[] contains 5 phrases (Yes, Doctor Visit, Within Last 6 Months ≥5? — yes, +5 included; Driver +5 included; No-lawyer +20).
  - **COLD walk (Self-table)**: timing=Within Last 6 Months (+5), injury=Still Being Evaluated (+10), treatment=Doctor Visit (+10), role=Driver (+5), insurance=Not Yet (0), work=No Impact (0), lawyer=Spoke With Lawyers (+15), contact=phone only = +10. Sum = 55. Wait — that's WARM. Adjust: timing=More Than 6 Months (0), injury=Not Sure Yet (+5), treatment=Doctor Visit (+10), role=Driver (+5), insurance=Not Yet (0), work=No Impact (0), lawyer=Spoke (+15), contact=phone+email (+15). Sum = 50. Expect classification `COLD` (Self-table COLD = [26,50] inclusive upper), lead_score 50.
  - **SPAM via score**: timing=More Than 6 Months (0), injury=Not Sure Yet (+5), treatment=Doctor Visit (+10), role=Driver (+5), all others 0 or negative, contact=email only (+5). Sum = 25. Expect classification `SPAM` (Self-table SPAM = [0,25] inclusive upper), lead_score 25.
  - **Family/Friend differs from Self**: same answers as the COLD walk above (sum = 50) but request_type = `Friend / Family Member`. Expect classification `WARM` (Family/Friend table treats 26–75 as WARM, no COLD bucket). Lead_score still 50. Asserts FR-006 / FR-039 / SC-005.
  - **All "I Don't Know" / 0-weight scenario**: every scoring question answered with the I Don't Know chip where available; for Yes/No questions answer the 0-weight option. timing=I Don't Know (0), injury=Not Sure Yet (+5), treatment=I Don't Know (0), role=I Don't Know (0), insurance=I Don't Know (0), work=I Don't Know (0), lawyer=I Don't Know (0), contact=phone+email (+15). Sum = 20. Expect classification `SPAM`, lead_score 20, reasons[] contains only contact-derived entries (Phone +10 ≥5, Email +5 ≥5) since all chip weights ≤4 are excluded by FR-010a.
  - **Negative-score-floored-at-0**: timing=More Than 6 Months (0), injury=No (-20), treatment=No Treatment (-10), role=Driver (+5), insurance=Not Yet (0), work=No Impact (0), lawyer=Yes I Have A Lawyer (-20), contact=blank (no phone, no email). Raw sum = -45 → floored to 0. Without hard-override (test toggles missing_contact OFF for this sub_type via direct DB UPDATE before the walk, so the score path is observable in isolation), expect classification `SPAM`, lead_score 0. Restore the hard-override toggle after the test. Per FR-005.

  Each scenario asserts: classification, lead_score, reasons[] (exact array), request_type, geographic_qualification. Re-walking the same scenario MUST produce identical outputs (FR-004 determinism gate).

  **Implementation notes**:
  - Each scenario gets its own `test()` block so failures are isolated.
  - Use a shared helper `walkAndAssert(page, scenario)` that takes a scenario object `{ chips: Record<step_slug, chip_slug>, contactFields, expected: { classification, lead_score, reasons } }` to keep scenarios declarative.
  - The "Negative-score-floored-at-0" scenario requires temporarily disabling the missing_contact hard-override — use a `beforeEach` / `afterEach` to set/restore via direct DB UPDATE on `sub_types.scoring_config_json`. Document this pattern in the spec file's header.
  - All 6 scenarios MUST run against the same dev seed; the ensure-car-accident-scoring remediation must have run beforehand (it has — see T013c).

### Implementation for User Story 1

- [X] T022 [US1] Edit `packages/api/src/lib/sop/state-machine.ts` `nextPendingStep` (around lines 289–302) to add the conditional skip per `research.md §R2`: if `step.applies_when_sub_type_slug` is set, look up the captured `sub_type` from `state.steps`, and skip the step unless slugs match. Make T014 pass. Touch nothing else; the existing default-step path (where the column is null) MUST continue to fire as before.

      **Result**: Filter implemented in 6 lines. Reads `state.steps[where slug === 'sub_type'].captured_value` (defensive — null if no sub_type captured). Skips any step whose `applies_when_sub_type_slug` is set and doesn't match. The 26 existing nextPendingStep tests + 4 new filtering tests = 30/30 pass; full state-machine test suite = 31/31 pass; full api suite = 458 tests; typecheck clean.

      Important runtime effect: this commit unblocks the dev server's visitor flow. Pre-T022, every visitor (including DUI/First Offense) would see the 9 car-accident scoring questions because nextPendingStep returned the first pending step regardless of sub_type. Post-T022, only personal_injury → car_accident visitors see them.
- [X] T023 [US1] Edit `packages/api/src/lib/leads.ts` `captureLead` (lines 71–159) to invoke `scoreLead` after the leads row INSERT is staged, BEFORE the actual write. The flow per `research.md §R7`: read `sub_types.scoring_config_json` for the captured sub_type; if non-null, call `scoreLead({sopState, scoringConfig, chipsBySlug})`; map `ScoredLead → leads` columns (classification, lead_score, score_reasons_json, request_type, geographic_qualification, geographic_qualification_details_json); apply hard-overrides per FR-010c (run AFTER the write succeeds to preserve "always capture" semantics). Wrap in try/catch per FR-010b: on throw, persist with `classification: 'SPAM', lead_score: null, score_reasons_json: '["scoring_error"]'` and emit ERROR-level log. Make T017 pass.

      **Result**: Implementation:
      - New module-level helper `computeScoringFields(accountId, sopState, contactPhone, contactEmail)` — async; returns ScoringResult or NULL_SCORING_RESULT.
      - Guard: returns NULL_SCORING_RESULT when sopState is null, sopState.is_finalized !== true, no case_type/sub_type captured, or sub_type lacks scoring_config_json (LLM fallback path per FR-022).
      - Looks up sub_type via account_id + case_type slug + sub_type slug to scope correctly per-account.
      - Parses scoring_config_json against scoringConfigSchema; on parse failure returns FR-010b safe-default {classification: 'SPAM', lead_score: null, reasons: ['scoring_error']}.
      - Builds chip catalog from sop_steps where applies_when_sub_type_slug = captured sub_type slug, parses each row's inline_chips_json, flattens into Map<slug, Chip>.
      - Computes contactBonus from xlsx Q8: phone +10 if digit-count >= 7, email +5 if matches a basic regex.
      - Calls scoreLead, builds reasons via buildReasons (no hard-overrides applied yet — Phase 6 / T064 wires those).
      - captureLead body: scoringFields computed once at top; finalClassification = scoringFields.classification ?? input.classification (rule-based wins per FR-001); used in INSERT, UPDATE, notification trigger, and return value. New columns written on both INSERT and UPDATE branches.

      Hard-override application is intentionally deferred to Phase 6 / T064 per the original task ordering. The structured log (T024) is also still pending. Both will follow before US1's walk spec runs.

- [X] T025 [US1] Edit `packages/api/src/lib/leads.ts` `updateLeadSOPState` (lines 185–259) to ALSO invoke `scoreLead` and the structured log path on the contact-form-driven finalization path. The two finalization entry points (LLM `captureLead` tool and contact-form-via-SOP-state) MUST produce identical lead-row shapes for the same SOP state. Make T018 pass.

      **Result**: Wired computeScoringFields into updateLeadSOPState after the existing contact-backfill block. Uses the post-backfill contact values (newPhone/newEmail) so the contactBonus reflects final form data. finalClassification overrides existingRow.classification only when the rule-based scorer applied; otherwise preserves the LLM-supplied value. New columns written in the same UPDATE statement as the contact backfill — single round-trip per finalization. T018's transition test passes.

      Both finalization paths (captureLead + updateLeadSOPState) now share computeScoringFields and produce identical lead-row shapes for the same SOPState, satisfying the FR-004 determinism contract.
- [X] T024 [US1] Edit `packages/api/src/lib/leads.ts` to emit the `lead_classified` structured log entry per `contracts/lead-finalization-log.md` immediately after the leads-row write (and BEFORE the urgent-lead notification path). Use `console.info(JSON.stringify(...))` for success path and `console.error(JSON.stringify(...))` for the scoring_error variant. Verify the log NEVER contains captured PII (name, phone digits, email contents, city/state). Add a unit test in `leads.test.ts` using `vi.spyOn(console, 'info')` asserting the exact log shape and asserting absence of PII fields. Per FR-034 / FR-010d.

      **Result**: All 5 T024 tests pass. Implementation:
      - Extended `ScoringResult` interface with `scoring_path: 'rule_based' | 'llm_fallback' | 'scoring_error'` and `scoring_error_detail: string | null` so the caller can distinguish paths and route info vs error log levels.
      - New helper `errToDetail(err)` converts an exception to a `Name: message` string. Excludes stack traces (could leak code paths) and never echoes captured PII.
      - New helper `emitLeadClassifiedLog({...})` builds the contract-specified JSON payload and routes to `console.info` (success paths) or `console.error` (scoring_error variant). Per FR-010d the payload only consumes already-resolved fields: lead_id, account_id, session_id, classification, lead_score, reasons, case_type_slug, sub_type_slug, hard_override_fired (placeholder until Phase 6 / T064), scoring_path, request_type, geographic_qualification, sop_version. The function NEVER reads from `input.name` / `input.contactEmail` / `input.contactPhone` / SOP free-text fields — it can't accidentally leak PII.
      - Wired into 3 emit sites: captureLead INSERT branch, captureLead UPDATE branch, updateLeadSOPState. All 3 emit identically-shaped payloads for the same SOPState.
      - Two scoring_error returns inside `computeScoringFields` (parse failure + scoreLead throw) now set `scoring_path: 'scoring_error'` and `scoring_error_detail`. The `_error` field is appended to the log payload only on the error variant.
      - PII test uses unique needle strings (`PII_NAME_NEEDLE_42`, `pii_email_needle@hidden.local`, `+1-555-PII-NEEDLE`) and greps the entire console.info call set; if any fragment of those needles appears anywhere in the serialized logs, the test fails. Per Constitution V.
      Full suite: 468 api tests (was 463 + 5 new); typecheck clean.
- [X] T025 [US1] Edit `packages/api/src/lib/leads.ts` `updateLeadSOPState` (lines 185–259) to ALSO invoke `scoreLead` and the structured log path on the contact-form-driven finalization path. The two finalization entry points (LLM `captureLead` tool and contact-form-via-SOP-state) MUST produce identical lead-row shapes for the same SOP state. Make T018 pass.

      **Result**: See T023's notes (committed together).
- [X] T026 [US1] Edit `packages/api/src/app/api/chat/route.ts` `captureLead` tool definition (lines 183–214) to update the parameter Zod schema's `classification` field from `z.enum(['urgent', 'normal', 'unqualified'])` to `z.enum(['HOT', 'WARM', 'COLD', 'SPAM'])`. Make T019 pass. Per `contracts/lead-classification-enum.md`.

      **Result**: Schema extracted to sibling `packages/api/src/app/api/chat/tool-params.ts` (Next.js route.ts rejects non-route exports). The route imports from there. Tool description updated to reference HOT/SPAM instead of urgent/unqualified. Coordinated changes:
      - `leads.ts:CaptureLeadInput.classification` type literal updated 3-value → 4-value
      - `leads.ts:wasNotUrgent`/`isNowUrgent` now compare against `'HOT'` instead of `'urgent'`; comment notes the notification `type` string stays `'urgent_lead'` for backward-compat per contract
      - `leads.ts:143` notification fires on `'HOT'` instead of `'urgent'`
      - `partial-lead.ts:classifyPartialLead` return type updated to the 4-value enum; rationale strings unchanged; intentionally no COLD path (heuristic signal coverage isn't fine-grained enough)
      - `system-prompt.ts:138-141` rubric replaced with 4-value vocabulary (T020/T027)
      - `leads.test.ts` and `partial-lead.test.ts` literals bulk-renamed (`'urgent'`→`'HOT'`, `'normal'`→`'WARM'`, `'unqualified'`→`'SPAM'`); 19 + 6 occurrences updated.
      Full suite: 454 api tests (was 441 + 13 new from T019 = 454); typecheck clean.
- [X] T027 [US1] Edit `packages/api/src/lib/system-prompt.ts` (lines 138–141) to replace the 3-line classification rubric with the 4-line rubric per `research.md §R6`. Token budget: the change is +0 net tokens (Constitution VI). Make T020 pass.

      **Result**: Rubric rewritten in T026's commit. 4 lines now (HOT/WARM/COLD/SPAM with descriptions matching `research.md §R6`). Token impact estimated at +30-50 tokens vs. the legacy 3-line block — well within the ~4500 budget per Constitution VI.
...
- [X] T081 [US7] Edit `packages/api/src/lib/partial-lead.ts` `classifyPartialLead` (lines 59–109) to update the return enum from `'urgent' | 'normal' | 'unqualified'` to `'HOT' | 'WARM' | 'COLD' | 'SPAM'`. Re-tune the regex-driven branch logic so the strongest urgency signal maps to `'HOT'`, valid-but-unfocused legal matter to `'WARM'` or `'COLD'` based on signal strength (use a simple heuristic: presence of incident description + contact intent → WARM; sparse signals → COLD), and no-legal-matter / no-contact-info → `'SPAM'`. Per `contracts/lead-classification-enum.md §Producers` item 3. Make T079 pass.

      **Result**: Folded into T026's enum-migration commit (the partial-lead heuristic is one of three classification producers; updating the enum required updating all three together to keep Constitution II's "single source of truth" intact). The 4-value mapping uses the coarse legacy semantics: HOT = urgency+legal signals, WARM = legal-only signals, SPAM = no-legal-signals. Intentionally NO COLD branch in MVP — the heuristic regex coverage isn't fine-grained enough to distinguish COLD from WARM, and a coarse-but-honest mapping is better than a fabricated one. T079 (the test for this) lives in Phase 9 and will assert this exact branch behaviour when it runs.
