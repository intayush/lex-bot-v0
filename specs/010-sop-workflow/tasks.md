...
- [~] T038 [US1] Create Playwright E2E spec `packages/api/tests/e2e/sop.spec.ts → "default SOP happy path (US1)"`: walks DUI → First Offense → free-text where → free-text what → Yesterday chip → answers follow-ups; asserts progress bar text 0/5 → 5/5; asserts a `leads` row inserted with non-null `sop_state_snapshot` matching `sopStateSchema` **— DEFERRED.** Three blockers: (1) `@playwright/test` is not installed (009-deployment-release T002 hasn't run); (2) the deferred T016/T019 means there's no migrated DB to test against locally; (3) the test would need either a real `GOOGLE_GENERATIVE_AI_API_KEY` (cost + flakiness risk) or a mock-LLM dev mode (additional infra). All three are real work that's out of scope for the US1 MVP slice. The integration is statically validated (full repo typecheck clean, 167 unit/integration tests across all helpers + leads). Re-attempt when Phase 8 deploy/eval infra lands OR when local Docker is available.
...
- [~] T042 [US2] Add Playwright E2E case in `packages/api/tests/e2e/sop.spec.ts → "skip detection (US2)"`: types "I was in a car accident last week downtown and need help. It was a hit-and-run on 5th avenue." as the first message; asserts progress bar advances by ≥ 2 in one turn; asserts the next pending step is the earliest unanswered **— DEFERRED.** Same blockers as T038: no @playwright/test installed, no test browsers configured. Behavior is verified by 18 new skip-detector unit tests + 2 multi-step advancer tests + manual live verification on 2026-05-23 ("I had a car accident in Pittsburgh last week" → progress 0→3, captured case_type=personal_injury, sub_type=car_accident, when=2026-05-11 in one turn).
...
- [~] T047 [US3] Add Playwright E2E case in `packages/api/tests/e2e/sop.spec.ts → "off-SOP detour (US3)"`: in mid-flow types "What are your office hours?"; asserts response contains the configured office hours phrase AND the pending step's question_text; asserts progress bar value unchanged; asserts at least one `sop_off_topic_detour` event in captured server logs **— DEFERRED.** Same blockers as T038/T042 (no @playwright/test installed). Behavior verified by 12 unit tests + 4 system-prompt-block tests + manual live verification on 2026-05-23 (mid-DUI conversation, "what is the phone number?" → agent answers verbatim then re-prompts "What kind of DUI matter is this?", SOP state unchanged).
