...
- [~] T070 [US6] Add Playwright E2E case in `packages/api/tests/e2e/sop.spec.ts → "lawyer configures custom SOP (US6)"`: signs in to dashboard; reorders a step via DnD (Playwright `dragTo`); adds a custom step; Save → asserts new `sop_configurations` row with `is_published=false`; Publish → asserts published flag flipped; opens fresh widget chat in a new context and asserts the new step list is in use **— DEFERRED.** Same blockers as T038/T042/T047/T056 (no `@playwright/test` installed, no `tests/e2e/` infra in the API package, no test browsers configured). The full save→publish→widget-uses-new-SOP loop IS verified end-to-end in this session via a Python Playwright script (`/var/folders/.../sop_verify_8d.py`): logs in, navigates via the sidebar, exercises Save Draft and Publish through the editor UI, confirms the public widget GET `/api/config` sees the new version, and confirms preview-mode picks up an unpublished draft. When Phase 8 R3 / 009-deployment-release T002 lands `@playwright/test` in the repo, this script becomes the basis for the formal E2E spec.
