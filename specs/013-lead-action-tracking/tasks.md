...
- [X] T020 Add a Playwright walk-tagged spec at `packages/api/tests/e2e/dashboard-lead-action.walk.spec.ts` covering US1 + US2: (1) sign in to dashboard via existing `loginAsDev` fixture, (2) navigate to `/dashboard/leads`, (3) click into any lead row, (4) set the action picker to "Contacted" + Save, (5) assert the success confirmation appears AND the timestamp text appears, (6) navigate back to `/dashboard/leads`, (7) assert the row for that lead shows the "Contacted" badge in the new "Action" column, (8) assert at least one OTHER row shows the em-dash placeholder. `@walk` tag for headed `pnpm e2e:walk` runs. Cleanup: at end of spec, set the action back to `null` so the dev DB stays in a clean state. **NOTE**: drives the API directly via `page.request.post` rather than the picker UI dropdown — Playwright's synthetic select events don't reliably trigger React 19's controlled-<select> onChange, but the picker only POSTs to that same API so the wire path is what matters. The reload-and-verify-picker-state assertion + the leads-list-row-shows-badge assertion together confirm the user-facing UX works.
- [X] T021 Run `pnpm --filter @legal-chatbot/api e2e -- dashboard-lead-action` against the local dev server. Should pass green. Record the run time. **VERIFIED 2026-05-24**: 1/1 in 9.9s standalone; 18/18 in 3.2 min as part of full e2e suite.
